...
const char kBEGIN_SATURN_V_GENERATED_CODE[] =
    "//// BEGIN ARBORETUM GENERATED CODE ////";
const char kEND_SATURN_V_GENERATED_CODE[] =
    "////   END ARBORETUM GENERATED CODE ////";

const char kDO_NOT_EDIT[] = "//////////// DO NOT EDIT //////////////";

template <size_t N>
std::optional<TextLocation> GetTextLocation(const std::string& text,
                                            const char query[N],
                                            size_t start_offset) {
  ...
  if (loc.line_end_offset == std::string::npos)
    loc.line_end_offset = text.size();
...
std::vector<GeneratedBlock> find_generated_blocks(const std::string& text) {
  ...
  while (offset < text.size()) {
    std::optional<TextLocation> start_banner =
        GetTextLocation<sizeof(kBEGIN_SATURN_V_GENERATED_CODE)>(
            text, kBEGIN_SATURN_V_GENERATED_CODE, offset);
    if (!start_banner.has_value()) break;

    std::optional<TextLocation> end_banner =
        GetTextLocation<sizeof(kEND_SATURN_V_GENERATED_CODE)>(
    ...
    result.push_back(GeneratedBlock{
        .begin_generated_location = std::move(start_banner.value()),
...
PartiallyGeneratedFile PartiallyGeneratedFile::Read(std::filesystem::path p) {
  ...
  std::vector<GeneratedBlock> generated_blocks =
      find_generated_blocks(input_file.value());
...
bool PartiallyGeneratedFile::Write(
    std::function<bool(std::ostream&)> output_fn) {
...

  {
    ...
    os << original_contents.substr(
        0, generated_block.begin_generated_location.line_end_offset + 1);

    if (!output_fn(os)) return false;

    os << original_contents.substr(
        generated_block.end_generated_location.line_start_offset);
  }

...
Id *SourceModel::resolve(clang::FileID fid) {
  clang::SourceManager &sm = ctx_.getSourceManager();
  auto file_entry = sm.getFileEntryRefForID(fid);
  if (!file_entry.has_value()) return data_model_.invalid_file_;
  ...
  // Filename
  arboretum_create_edge(file, data_model_.file_name_,
                        data_model_.arboretum_node_for(std::filesystem::canonical(name).c_str()));

  // Content (if available)
  auto content = sm.getBufferDataOrNone(fid);
  if (content.has_value()) {
    arboretum_create_edge(file, data_model_.file_content_, data_model_.arboretum_node_for(content->data()));
  }

  return file;
}

Id *SourceModel::resolve(clang::SourceLocation source_location) {
  clang::SourceManager &sm = ctx_.getSourceManager();
  void *key = source_location.getPtrEncoding();

  if (!source_location.isValid()) return data_model_.invalid_source_location_;
  ...
  arboretum_create_edge(source_loc, data_model_.meta_class_, data_model_.source_location_class_);
  arboretum_create_edge(source_loc, data_model_.source_location_is_file_,
  ...
  if (source_location.isFileID()) {
    ...
    arboretum_create_edge(source_loc, data_model_.source_location_file_, resolve(fid));
    ...
    if (!PLoc.isInvalid()) {
      arboretum_create_edge(source_loc, data_model_.source_location_line_,
                            data_model_.arboretum_node_for(static_cast<uint64_t>(PLoc.getLine())));
      arboretum_create_edge(source_loc, data_model_.source_location_column_,
                            data_model_.arboretum_node_for(static_cast<uint64_t>(PLoc.getColumn())));
    }
  }

  const Id *expansion_loc = resolve(sm.getExpansionLoc(source_location));
  if (expansion_loc != source_loc) {
    arboretum_create_edge(source_loc, data_model_.source_location_expansion_loc_, expansion_loc);
  }

  const Id *spelling_loc = resolve(sm.getSpellingLoc(source_location));
  if (spelling_loc != source_loc) {
    arboretum_create_edge(source_loc, data_model_.source_location_spelling_loc_, spelling_loc);
  }

  return source_loc;
}

Id *SourceModel::resolve(clang::SourceRange source_range) {
  ...
  arboretum_create_edge(result, data_model_.meta_class_, data_model_.source_range_class_);
